...
class FestivalSelectScreen extends StatelessWidget {
  ...
  Future<List<dynamic>> fetchFestivals() async {
    final response =
        await supabase.from('festivals').select().eq('active', true);
    if (response != null) {
      print(response);
        ...
        child: SingleChildScrollView(
          ...
          child: Column(
            mainAxisSize: MainAxisSize
                .min, // Ensure the column's content is centered vertically
                ...
                builder: (context, snapshot) {
                  if (snapshot.connectionState == ConnectionState.waiting) {
                    ...
                    return ListView.builder(
                      shrinkWrap:
                          true, // Makes the ListView take up only as much space as it needs
                      physics:
  ...
  Future<void> setOnboardingStatus(String festivalName, festivalLogo,
      festivalDBPrefix, festivalId, bool status) async {
    SharedPreferences prefs = await SharedPreferences.getInstance();
